...
// Labels des genres
// Ordre d'affichage des genres (personnalisable)
const genreOrder = [
  'editorial',
  'reportage',
  'interview',
...
async function loadArticles() {
  try {
    // Réinitialiser complètement articlesData avant de charger
    articlesData = {
      editorial: [],
      reportage: [],
      interview: [],
...
function migrateData(data) {
  ...
  // Copier les genres déjà au bon format
  if (data.editorial) newData.editorial = [...data.editorial];
  if (data.reportage) newData.reportage = [...data.reportage];
  if (data.interview) newData.interview = [...data.interview];
  if (data.enquete) newData.enquete = [...data.enquete];
  if (data.analyse) newData.analyse = [...data.analyse];
  if (data.billet) newData.billet = [...data.billet];

  // Migrer analyses vers analyse (ancien format)
  if (data.analyses) {
    newData.analyse = [...newData.analyse, ...data.analyses.map(a => ({ ...a, genre: 'analyse' }))];
  }

  // Copier temoignages (format actuel)
  if (data.temoignages) {
    newData.temoignages = [...data.temoignages];
  }

  // Migrer actualites vers reportage (ancien format)
  if (data.actualites) {
    newData.reportage = [...newData.reportage, ...data.actualites.map(a => ({ ...a, genre: 'reportage' }))];
  }

  // Migrer ressources vers enquete (ancien format)
  if (data.ressources) {
    newData.enquete = [...newData.enquete, ...data.ressources.map(a => ({ ...a, genre: 'enquete' }))];
  }
...
function updateStats() {
  ...
  // Générer les statistiques selon l'ordre défini
  const genreStats = genreOrder.map(genre => {
    if (articlesData[genre]) {
      return `
        <div class="stat-card">
          <h3>${genreLabels[genre] || genre}</h3>
          <p class="number">${articlesData[genre].length}</p>
        </div>
      `;
    }
    return '';
  ...
  const filterArticles = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(a =>
        ...
        (a.content && (Array.isArray(a.content) ? a.content.join(' ') : a.content).toLowerCase().includes(searchTerm))
      );
    }
...
// Convertir le HTML de Quill en format de stockage (tableau de paragraphes HTML)
function quillHtmlToContent(html) {
  if (!html || html === '<p><br></p>' || html.trim() === '') return [];
  
  // Diviser le HTML en éléments (paragraphes, titres, listes, etc.)
...
// Convertir le contenu stocké en HTML pour Quill
function contentToQuillHtml(content) {
  if (!content) return '';
...
function editArticle(id, genre) {
  ...
  // Charger le contenu dans Quill
  const htmlContent = contentToQuillHtml(article.content);
...
function saveFolderPermissions(projectHandle, projectPath) {
  try {
    ...
    const normalizedPath = (projectPath || window.location.pathname).replace(/\\/g, '/');
...
async function selectProjectFolder() {
  ...
  try {
    ...
    // Sur Windows, essayer de commencer dans "documents" ou "downloads"
    try {
...
async function saveArticles() {
  // Sauvegarder directement avec les genres standards
...
// Générer les options de genre selon l'ordre défini
function generateGenreOptions() {
  return genreOrder.map(genre => 
    `<option value="${genre}">${genreLabels[genre] || genre}</option>`
  ).join('');
}

// Gestion du formulaire
document.addEventListener('DOMContentLoaded', function() {
  // Initialiser les selects avec l'ordre personnalisé
  const articleTypeSelect = document.getElementById('article-type-select');
  const filterTypeSelect = document.getElementById('filter-type');
  
  if (articleTypeSelect) {
    articleTypeSelect.innerHTML = generateGenreOptions();
  }
  
  if (filterTypeSelect) {
    filterTypeSelect.innerHTML = '<option value="">Tous les genres</option>' + generateGenreOptions();
  }
  ...
  form.addEventListener('submit', function(e) {
    ...
    // Récupérer le contenu HTML de Quill et le convertir en format de stockage
    const quillHtml = quillEditor.root.innerHTML;
    const content = quillHtmlToContent(quillHtml);
    ...
    if (content.length === 0 || quillHtml === '<p><br></p>') {
      showError('Veuillez ajouter du contenu à l\'article.');
      return;
    }
